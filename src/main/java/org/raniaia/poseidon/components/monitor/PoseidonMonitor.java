package org.raniaia.poseidon.components.monitor;
 ...
 *
 * 这个监控器是面向所有对象而开发的，如果你想在你自己的对象中添加监控，你只需要将对象继承{@link PoseidonMonitor}
 * 即可。然后调用{@link PoseidonMonitor#createMonitorPage(String)}创建一个新的监控页面即可。
 *
 * 默认是将对象中的成员转换成表字段进行显示，如果你需要自己创建字段可以使用{@link
 * PoseidonMonitor#createMonitorPage(String, String...)}自定义字段。
 *
 * English:
 *
 ...
 *
 * This {@link PoseidonMonitor} can extension but you need import poseidon extension develop package.
 *
 * If you want reset {@code ConnectionPool} the max connection or min connection or else config,
 * you can open <b>Poseidon PoseidonMonitor Page</b> in browse, default url is "http://127.0.0.1:8080/poseidon/monitor".
 *
 * This {@link PoseidonMonitor} can use in everywhere, if you the object need use monitor, you can extends
 * {@link PoseidonMonitor} object then calling {@link #createMonitorPage(String)} method create new monitor page.
 *
 ...
 * @author tiansheng
 */
public interface PoseidonMonitor {
