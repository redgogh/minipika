package org.raniaia.minipika.framework.jap;
...
public class JapLoader {
    ...
    public Map<String,Map<String,String>> load(){
        return load("resources/minipika.jap");
    }
    ...
    public Map<String,Map<String,String>> load(String path) {
        ...
        if(!GlobalConfig.isJar()) {
            ...
            try {
                configContent = MinipikaIOUtils.getStringByInputStream(new FileInputStream(System.getProperty("user.dir").concat(path)));
            } catch (Exception e) {
                e.printStackTrace();
            }
        }
