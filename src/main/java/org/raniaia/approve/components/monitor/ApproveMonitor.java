package org.raniaia.approve.components.monitor;
...
/**
 *
 * Approve Framework系统监控器, 负责监控系统运行时的执行数据，它可以帮助开发者
 ...
 *
 * 开发者可以对监控器对象进行程序扩展来达到自己的目的，但是需要引入Approve的扩展开发包。
 *
 * 监控器页面的默认URL路径: http://127.0.0.1:8080/approve/monitor
 *
 * 这个监控器是面向所有对象而开发的，如果你想在你自己的对象中添加监控，你只需要将对象继承{@link ApproveMonitor}
 * 即可。然后调用{@link ApproveMonitor#createMonitorPage(String)}创建一个新的监控页面即可。
 *
 * 默认是将对象中的成员转换成表字段进行显示，如果你需要自己创建字段可以使用{@link
 * ApproveMonitor#createMonitorPage(String, String...)}自定义字段。
 *
 * English:
 *
 * System monitor for approve framework it can record system
 ...
 *
 * This {@link ApproveMonitor} can extension but you need import approve extension develop package.
 *
 * If you want reset {@code ConnectionPool} the max connection or min connection or else config,
 * you can open <b>Approve ApproveMonitor Page</b> in browse, default url is "http://127.0.0.1:8080/approve/monitor".
 *
 * This {@link ApproveMonitor} can use in everywhere, if you the object need use monitor, you can extends
 * {@link ApproveMonitor} object then calling {@link #createMonitorPage(String)} method create new monitor page.
 *
 ...
 * @author tiansheng
 */
public interface ApproveMonitor {
